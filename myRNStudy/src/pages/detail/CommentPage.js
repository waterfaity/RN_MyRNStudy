...
import { ActivityIndicator, FlatList, Image, ListRenderItemInfo, RefreshControl, Text, TextInput, View, Pressable } from 'react-native';
        ...
        renderItem={ (itemInfo: ListRenderItemInfo<CommentBean>) => {
          ...
          return <Pressable onPress={ () => {
            ...
            <View style={ { paddingLeft: 15, paddingRight: 15, paddingTop: 10 } }>
              ...
              {/*用户名*/ }
              <Text style={ { marginLeft: 50, fontSize: 18, color: TextColorMain } }>{ itemInfo.item.userNickName }</Text>
              {/*创建时间*/ }
              <Text style={ { marginLeft: 50, fontSize: 13, color: TextColorLight } }>{ itemInfo.item.createTime }</Text>
              {/*评论内容*/ }
              <Text style={ { marginLeft: 50, marginTop: 10, fontSize: 14, color: TextColorLight } }>{ itemInfo.item.content }</Text>
              {/*点赞数*/ }
